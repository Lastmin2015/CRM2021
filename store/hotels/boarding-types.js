export default {
  ...
  getters: {
    boardingTypes: state => state.boardingTypes
  },

  mutations: {
    SET_BOARDING_TYPES (state, payload) {
      state.boardingTypes = payload
    }
  },
  ...
  actions: {
    async get ({ commit }) {
      try {
        ...
        const { result } = await this.$axios.$get('/api/boarding-types?with_data=true')
        commit('SET_BOARDING_TYPES', result ?? [])
